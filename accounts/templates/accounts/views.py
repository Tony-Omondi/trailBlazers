...
def submit_order(token, ipn_id, cart_obj):
    global merchant_reference
    merchant_reference = random.randint(1, 1000000000000000000)
    amount = 1  # Get the amount from cart
    callback_url = "http://127.0.0.1:8000/accounts/success/?order_id={{ payment.id }}"
...
# Main flow for creating a purchase
def make_purchase(request):
    if request.method == "POST":
        ...
        # Step 3: Get the cart object and submit the order
        cart_obj = Cart.objects.get(is_paid=False, user=request.user)
        order_response = submit_order(token, ipn_id, cart_obj)
        if order_response and order_response.get('status') == "200":
            cart_obj.razorpay_order_id = order_response.get('order_tracking_id')  # Set order ID from response
            cart_obj.save()
...
def cart(request):
    ...
    if cart_obj:
        ...
        payment = {'amount': cart_total_in_paise, 'currency': 'KES', 'payment_capture': 1}
        # cart_obj.razorpay_order_id is updated after submitting order
